...
type IngestionService interface {
	...
	UploadGeoJSON(ctx context.Context, userID, filename string, data []byte, contentType string) (string, error)
	CreateJob(ctx context.Context, userID, regionID, jobType string, params *models.JobInferenceParams) (*models.Job, error)
}
...
// UploadGeoJSON stores a GeoJSON file in the user's bucket folder.
func (s *Service) UploadGeoJSON(ctx context.Context, userID, filename string, data []byte, contentType string) (string, error) {
	if s.storage == nil {
	...
	path := fmt.Sprintf("%s/%d-%s", userID, time.Now().UTC().UnixNano(), safeName)

	uploadedPath, err := s.storage.UploadFile("geojson-uploads", path, data, contentType)
	if err != nil {
		return "", fmt.Errorf("upload geojson %q: %w", filename, err)
	}
...
// If params is non-nil the SentinelHub parameters are encoded into result_refs so the
// orchestrator can pass them to the inference server when dispatching the job.
func (s *Service) CreateJob(ctx context.Context, userID, regionID, jobType string, params *models.JobInferenceParams) (*models.Job, error) {
	...
	if params != nil {
		refs := &models.JobResultRefs{Params: params}
		encoded, err := json.Marshal(refs)
		if err != nil {
			return nil, fmt.Errorf("encode job params: %w", err)
		}

		job.ResultRefs = encoded
	}
