...
func New(reports repository.ReportRepository, jobs repository.JobRepository, storage StorageClient) ReportService {
	return &Service{reports: reports, jobs: jobs, storage: storage}
}
...
func (s *Service) GenerateReport(ctx context.Context, userID, jobID, reportType string) (*models.Report, error) {
	...
	if s.storage != nil {
		payload, payloadErr := buildReportPayload(job, reportType)
		if payloadErr != nil {
...
func buildReportPayload(job *models.Job, reportType string) ([]byte, error) {
	if job == nil {
