...
// FetchAndRunRequest is the payload sent to the inference server to start a job.
type FetchAndRunRequest struct {
	AOIBbox       [4]float64 `json:"aoi_bbox"`
...
// FetchAndRun submits a SentinelHub fetch-and-run job to the inference server.
// Transient network errors and 5xx responses are retried with exponential
// backoff; 4xx responses are returned immediately.
func (c *Client) FetchAndRun(ctx context.Context, req FetchAndRunRequest) (*InferenceJobResponse, error) {
	body, err := json.Marshal(req)
	if err != nil {
		return nil, fmt.Errorf("marshal fetch-and-run request: %w", err)
	}
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		result, retry, err := c.tryFetchAndRun(ctx, body)
		if err == nil {
			return result, nil
		}
...
func (c *Client) tryFetchAndRun(ctx context.Context, body []byte) (*InferenceJobResponse, bool, error) {
	httpReq, err := http.NewRequestWithContext(ctx, http.MethodPost, c.baseURL+"/api/jobs/fetch-and-run", bytes.NewReader(body))
	if err != nil {
		return nil, false, fmt.Errorf("build fetch-and-run request: %w", err)
	}
	...
	if err != nil {
		...
		return nil, true, fmt.Errorf("call inference server fetch-and-run: %w", err)
	}
	defer resp.Body.Close()
	...
	if err := json.NewDecoder(resp.Body).Decode(&result); err != nil {
		return nil, false, fmt.Errorf("decode fetch-and-run response: %w", err)
	}
