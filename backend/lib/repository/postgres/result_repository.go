...
func (r *resultRepository) SaveSegmentation(ctx context.Context, result *models.SegmentationResult) (*models.SegmentationResult, error) {
	...
	row := r.pool.QueryRow(ctx, `
		INSERT INTO public.segmentation_results (job_id, mask_path, pixel_count)
		VALUES ($1, $2, $3)
		RETURNING id, job_id, mask_path, pixel_count, created_at
	`, result.JobID, result.MaskPath, result.PixelCount)
...
func (r *resultRepository) SaveClassification(ctx context.Context, result *models.ClassificationResult) (*models.ClassificationResult, error) {
	...
	row := r.pool.QueryRow(ctx, `
		INSERT INTO public.classification_results (segmentation_id, patch_id, road_label, confidence)
		VALUES ($1, $2, $3, $4)
		RETURNING id, segmentation_id, patch_id, road_label, confidence, created_at
	`, result.SegmentationID, result.PatchID, result.RoadLabel, result.Confidence)
...
func (r *resultRepository) SaveConnectivityGraph(ctx context.Context, result *models.ConnectivityGraph) (*models.ConnectivityGraph, error) {
	...
	row := r.pool.QueryRow(ctx, `
		INSERT INTO public.connectivity_graphs (job_id, metrics)
		VALUES ($1, $2)
		RETURNING id, job_id, metrics, created_at
	`, result.JobID, result.Metrics)
...
func (r *resultRepository) GetResultsByJob(ctx context.Context, jobID string) (*models.JobResults, error) {
	...
	segRow := r.pool.QueryRow(ctx, `
		SELECT id, job_id, mask_path, pixel_count, created_at
	...
	if err == nil {
		...
		rows, clsErr := r.pool.Query(ctx, `
			SELECT id, segmentation_id, patch_id, road_label, confidence, created_at
	...
	connectivityRow := r.pool.QueryRow(ctx, `
		SELECT id, job_id, metrics, created_at
