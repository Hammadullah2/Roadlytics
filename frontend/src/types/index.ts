...
export interface JobInferenceParams {
  start_date: string;
  end_date: string;
...
export interface JobDownloads {
  ...
  seg_mask_tif?: string;
  roads_raw_shp?: string;
  roads_classified_shp?: string;
  roads_classified_csv?: string;
  graph_graphml?: string;
  graph_geojson?: string;
...
export interface BackendClassificationResult {
  ...
  confidence: number;
  bbox?: Feature<Geometry | null>;
  geometry?: Feature<Geometry | null> | Geometry | null;
  created_at: string;
}
...
const EMPTY_FEATURE: Feature<Geometry | null> = {
  type: "Feature",
  geometry: null,
  properties: {},
};
...
const isFeature = (value: unknown): value is Feature<Geometry | null> => {
...
const toFeature = (
  value: BackendClassificationResult["bbox"] | BackendClassificationResult["geometry"],
...
export const normalizeClassificationResult = (result: BackendClassificationResult): ClassificationResult => {
  return {
    ...
    bbox: toFeature(result.bbox ?? result.geometry),
    created_at: result.created_at,
  };
};
