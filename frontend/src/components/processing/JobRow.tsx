...
export const JobRow = ({ job }: JobRowProps): JSX.Element => {
  // Live updates come from Supabase Realtime — the inference server writes
  // progress directly to the jobs table, so no backend WebSocket is needed.
  const liveJobID = job.status === "pending" || job.status === "running" ? job.id : "";
  const realtime = useJobRealtime(liveJobID);

  const liveProgress = realtime.job?.progress ?? job.progress;
  const liveStatus = realtime.job?.status ?? job.status;
  const liveStage = deriveStage(liveProgress, liveStatus);
  const liveError = realtime.job?.error_message ?? job.errorMessage ?? null;
